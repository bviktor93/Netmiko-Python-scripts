...
commands = ['ntp server 99.99.99.99', 'snmp-server host 99.99.99.99 traps ver 2c TESTING']
show_cmds = ['do show ntp ass', 'do show snmp host']
...
for device in routers:
    try:
        ...
        net_connect.enable()
        hostname = net_connect.send_command("show run | sec hostname").split()[1]
        output = net_connect.send_config_set(show_cmds)
        print("\n" + " " * 30 + f"Output of the show commands on device {hostname}:")
        print("-" * 90)
        print(output)
        net_connect.disconnect()
    except NetmikoTimeoutException:
