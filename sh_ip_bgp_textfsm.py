...
for device in routers:
    try:
        ...
        output = net_connect.send_command("show ip bgp | begin Network")
        print("\n" + " " * 30 + f"Output of the device {hostname}:")
        print("-" * 90)
        print(output)
        net_connect.disconnect()
        with open("cisco_ios_show_ip_bgp.textfsm") as temp:
            fsm = textfsm.TextFSM(temp)
            result = fsm.ParseText(output)
        print("Headers in the template:")
        print(fsm.header)
        print("Parsed data:")
        print(result)
        print("The following prefixes have the 'i' Origin code:")
        for i in result:
            if i[9] == "i":
                print(i[3])
        print("The following prefixes have been redistributed into BGP:")
        for i in result:
            if i[9] == "?":
                print(i[3])
